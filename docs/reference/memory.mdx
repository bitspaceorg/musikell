...
| Module                      | Description                                                                            |
| --------------------------- | -------------------------------------------------------------------------------------- |
| [Buffer](memory/buffer.mdx) | `IOVector Float` wrapper with safe/unsafe sample access, copy, FFI pointer export      |
| [Layout](memory/layout.mdx) | Alignment and channel-aware size calculations, validation                              |
| [Pool](memory/pool.mdx)     | Pre-allocated `IntMap Buffer` keyed by `nodeId * 1000 + portId`, feedback buffer pairs |
